...
async function onrequest(req, res) {
    ...
    try {
        ...
        if (!success)
            return requestAuthorization(req, res, this);
    }
    catch (_err) {
        const err = _err;
    ...
    if (parsed.protocol !== 'http:') {
        ...
        res.writeHead(400);
        res.end(`Only "http:" protocol prefix is supported (got: "${parsed.protocol}")\n`);
        return;
    }
 ...
 */
async function authenticate(server, req) {
    if (server.authenticate === 'negotiate') {
        debug.request('authenticating request (negotiate) "%s %s"', req.method, req.url);
 ...
 */
function requestAuthorization(req, res, server) {
